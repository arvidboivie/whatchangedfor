...
class PatchNoteParser {
  ...
  public async parse() {
    ...
    console.log(latestVersionParsed);

    if (latestVersionParsed) {
      // this.patchList.findIndex(patch => patch.patch_number === latestVersionParsed)
      // slice patchlist down to only new patches
    }
  ...
  private saveChangesToDynamo(hero: HeroChanges, patch: Patch) {
    ...
    return this.dynamoClient.put(this.toSnakeCase(name), patch.patch_number, {
      patchDate: new Date(patch.patch_timestamp).toUTCString(),
      ...changes,
    });
  }
